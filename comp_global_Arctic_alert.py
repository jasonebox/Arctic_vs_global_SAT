...
def regressx_hybrid(x,y,do_time_series_plot,color,label):
    ...
    # temp_x=x[v]
    # temp_y=y[v]
    # b, m = polyfit(temp_x,temp_y, 1)
    # if do_time_series_plot:
    #     ax.plot(temp_x, b + m * temp_x, '--',c=color,linewidth=3)
...
# for ll in range(iyear+15,fyear-13):
    ...
    Global_change=regressx(x,y,do_time_series_plot,'b','global')
    ...
    Arctic_change=regressx_hybrid(x,y,do_time_series_plot,'r','Arctic')
    ...
    if do_time_series_plot:
        ...
        ylab='degrees C relative to '+str(year0_baseline)+'-'+str(year1_baseline)
        plt.ylabel(ylab)
        plt.title('Arctic and Global Air Temperature Anomalies')
        ...
        mult=0.8
        c0=0.3
        ax.text(0.7,yy0, tag, transform=ax.transAxes,
                fontsize=font_size*mult,verticalalignment='top', bbox=props,color=[c0,c0,c0], rotation_mode="anchor")
